      ...
      <Card className="max-w-4xl mx-auto shadow-xl glass-effect">
        ...
        <CardContent className="space-y-6">
          <div className="flex gap-2">
            <button onClick={()=>setTab("text")} className={`flex-1 rounded-md border px-4 py-2 ${tab==="text"?"bg-blue-600 text-white":"bg-white hover:bg-gray-100"}`}>
              <FileText className="h-4 w-4 mr-2 inline" /> Analyze Text
            </button>
            <button onClick={()=>setTab("url")} className={`flex-1 rounded-md border px-4 py-2 ${tab==="url"?"bg-blue-600 text-white":"bg-white hover:bg-gray-100"}`}>
              <Link2 className="h-4 w-4 mr-2 inline" /> Analyze URL
            </button>
          </div>

          {tab==="text" ? (
            <Textarea placeholder="Paste the text you want to analyze..." value={text} onChange={(e)=>setText(e.target.value)} />
          ):(
          ...
          <button onClick={analyze} disabled={loading || (!text.trim() && !url.trim())}
            ...
            {loading ? <><Brain className="h-5 w-5 mr-2 inline animate-pulse"/> Analyzing...</> : <><Search className="h-5 w-5 mr-2 inline"/> Analyze for Misinformation</>}
          </button>
          ...
          {result && (
            <div className="mt-4 space-y-6 p-6 bg-gray-50 rounded-lg">
                ...
                <>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{loading?"Analyzing...":"Analysis complete"}</p>
                      </div>
                    </div>
                  ...
                  {result.factors && (
                    <div className="space-y-3">
                      <h4 className="font-semibold flex items-center gap-2"><TrendingUp className="h-4 w-4 text-blue-600"/> Trust Factors</h4>
                  ...
                  {result.findings && (
                    <div className="space-y-3">
                      <h4 className="font-semibold flex items-center gap-2"><Eye className="h-4 w-4"/> Key Findings</h4>
                      <ul className="space-y-2 text-sm">
                        {result.findings.map((f,i)=>(
                          <li key={i} className="flex items-start gap-2"><CheckCircle className="h-4 w-4 text-green-500 mt-0.5"/>{f}</li>
                        ))}
                      </ul>
                    </div>
                  )}
                  ...
                  {result.sources && (
                    <div className="space-y-3">
                      <h4 className="font-semibold flex items-center gap-2"><Globe className="h-4 w-4"/> Verified Sources</h4>
                      <div className="grid gap-2">
                        {result.sources.map((s,i)=>(
                          <div key={i} className="flex items-center justify-between p-2 bg-white rounded border">
                            <span className="font-medium">{s.name}</span>
                            <Badge variant={s.credibility==="high"?"default":"secondary"}>{s.credibility} credibility</Badge>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              {/* Show raw result for debugging if missing expected fields */}
              {!result.status && !result.error && (
                <pre className="text-xs text-gray-500 bg-gray-100 p-2 rounded overflow-x-auto">{JSON.stringify(result, null, 2)}</pre>
              )}
            </div>
          )}
        </CardContent>
